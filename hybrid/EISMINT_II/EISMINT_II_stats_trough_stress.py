...
class Surface(Expression):
  ...
  w     = 200.0e3 # through width
  slope = b0/L
  def eval(self,values,x):
    if (x[0] > 0 and abs(x[1]) < self.w):
      values[0] = + thklim + self.b0 \
                  - max(0.0, self.slope * x[0] * cos(pi*x[1] / self.w))
    else: 
      values[0] = thklim + self.b0
